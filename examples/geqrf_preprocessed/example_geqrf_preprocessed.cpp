...
// that H b = [ 0 0 ... 0 *]', where the modulus of * is the 2-norm of b,
...
void run(size_t m, size_t n)
{
    ...
    auto startQR = std::chrono::high_resolution_clock::now();
    {
        ...
        tlapack::larf(tlapack::Side::Left, tlapack::Direction::Backward,
                      tlapack::StoreV::Columnwise, b, tau_b, A);
        ...
        // 4) Compute the last column of Q by applying the Householder
        //    reflectors to the last column of the identity matrix

        // 4.1) Set x to be the last column of the identity matrix
        for (idx_t i = 0; i < n; ++i)
    ...
    // Compute FLOPS
    // 6m + 4m*n + 2*m*n² - 2/3*n³ + 4m*n + n = 2*m*n² - 2/3*n³ + 8m*n + 6m
    double flopsQR =
        (2.0e+00 * ((double)m) * ((double)n) * ((double)n) -
         2.0e+00 / 3.0e+00 * ((double)n) * ((double)n) * ((double)n) +
